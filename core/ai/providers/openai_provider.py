...
_logger = get_logger("ai.openai")


class OpenAIProvider(AIProvider):
    """Provider that talks to the OpenAI Chat Completions API.

    Also works with any OpenAI-compatible endpoint (e.g. Azure, Together,
    Groq, local vLLM) by changing *base_url*.
    """
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if e.code == 429 and attempt < self.MAX_RETRIES - 1:
                    ...
                    time.sleep(delay)
                    continue
                _logger.error("OpenAI chat error", error=str(e))
                return f"[Error] {e}"
        ...
        for attempt in range(self.MAX_RETRIES):
            try:
                ...
                if e.code == 429 and attempt < self.MAX_RETRIES - 1:
                    ...
                    time.sleep(delay)
                    continue
                _logger.error("OpenAI stream error", error=str(e))
                yield f"\n[Error] {e}"
                return
            except Exception as e:
                _logger.error("OpenAI stream error", error=str(e))
